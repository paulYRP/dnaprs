process BUILD_CT_WEIGHTS {
    ...
    input:
    tuple val(target), val(meta), path(cojo), path(clumps), path(harmonisation_qc), path(clump_log)
    path weight_script

    output:
    tuple val(target), val(meta), path("${meta.trait_id}.plink_ct.weights.tsv"), path("${meta.trait_id}.plink_ct.weight_qc.tsv"), path(harmonisation_qc), path(clump_log), emit: weights
    ...
    printf 'cohort\ttrait_id\tprs_name\tharmonised_variants\tclumped_variants\tretained_percent\tstatus\n${target.cohort}\t${meta.trait_id}\t${meta.prs_name}\t1\t1\t100\tPASS\n' > ${meta.trait_id}.plink_ct.weight_qc.tsv
    """
}
